package optimisation.problems.cec2005;
...
public class benchmark {
	...
	public test_func testFunctionFactory(int func_num, int dimension) {
		...
		try {
			returnFunc = (test_func )
				loader.loadClass("optimisation.problems.cec2005."+test_func_class_names[func_num-1])
	...
	static public void loadTestDataFromFile(String file, int num_test_points, int test_dimension, double[][] x, double[] f) {
		try {
			BufferedReader brSrc = new BufferedReader(new FileReader(benchmark.class.getResource(file).getFile()));
