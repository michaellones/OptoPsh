...
public abstract class GA implements Serializable {
	...
	protected boolean _lexicaseSelection;
	...
	protected boolean GetBooleanParamWithDefault(String name, boolean defaultVal) throws Exception {
		if(!ParamExists(name))
			return defaultVal;
	...
	protected void InitFromParameters() throws Exception {
		...
		String lexicase = GetParam("lexicase-selection", true);
		_lexicaseSelection = (lexicase==null) ? false : 
			(lexicase.equalsIgnoreCase("true") ? true : false);
	...
	/**
	 * Performs either tournament or lexicase selection depending on parameters.
	 * Arguments are ignored if lexicase selection is used.
	 */
	
	protected GAIndividual Select(int inSize, int inIndex) {
		if(_lexicaseSelection)
			return LexicaseSelect();
		else
			return TournamentSelect(inSize, inIndex);
	}
	
	/**
	 * Performs a lexicase selection.
	 */
	
	protected GAIndividual LexicaseSelect() {
		int popsize = _populations[_currentPopulation].length;

		int cases = this._populations[_currentPopulation][0]._errors.size();
		
		//System.out.print("* ");
		// randomly re-order the fitness cases
		List<Integer> order = new ArrayList<Integer>(cases);
		for(int i=0; i<cases; i++)
			order.add(i);
		Arrays.asList(order);
		Collections.shuffle(order);
		
		List<GAIndividual> prev = Arrays.asList(_populations[_currentPopulation]);
		List<GAIndividual> best;
		float fbest, curr;
		
		for(int fcase : order) {
			fbest = Float.MAX_VALUE;
			best = new ArrayList<GAIndividual>();
			
			for(GAIndividual ind : prev) {
				curr = ind._errors.get(fcase); 
				if(curr < fbest) {
					best.clear();
					fbest = curr;
				}
			...
			//System.out.print(fcase+":"+best.size()+" ");
			if(best.size()==1) {
				//System.out.println(best.get(0).hashCode()+" "+best.get(0)._fitness);
				return best.get(0);
			}
		...
		// randomly select one from the remaining bunch
		GAIndividual selected = prev.get(new Random().nextInt(prev.size()));
		
		//System.out.println(selected.hashCode()+" "+selected._fitness);
		return selected;
	}
	
	/**
	 * Performs a tournament-based lexicase selection.
	 */
	
	protected GAIndividual TournamentLexicaseSelect() {
		int popsize = _populations[_currentPopulation].length;
		...
		List<GAIndividual> prev = Arrays.asList(_populations[_currentPopulation]);
		List<GAIndividual> best;
		GAIndividual tbest = null;
		float fbest, curr;
		Random rnd = new Random();
		
		int NMAX = prev.size() / 10;
		for(int fcase : order) {
			best = new ArrayList<GAIndividual>();
			
			if(prev.size()>_tournamentSize*2) {
				// select through tournament selection
				for(int i=0; i<NMAX; i++) {
					fbest = Float.MAX_VALUE;
					for(int t=0; t<_tournamentSize; t++) {
						GAIndividual choice = prev.get(rnd.nextInt(prev.size()));
						if(choice._errors.get(fcase)<fbest) {
							fbest = choice._errors.get(fcase);
							tbest = choice;
						}
					}
					best.add(tbest);
	...
	protected GAIndividual ReproduceByClone(int inIndex) {
		return Select(_tournamentSize, inIndex).clone();
	}
