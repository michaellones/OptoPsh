...
public class InspectorInput {
	...
	private void InitInspectorInput(File inFile) throws Exception {
		...
		if(programString.contains("{")) {
			_extras = new HashMap<String,String>();
			String[] extras = programString.substring(programString.indexOf('{')+1).split(",");
			String[] parts;
			for(String extra : extras) {
				parts = extra.split("=");
				_extras.put(parts[0], parts[1]);
			}
			programString = programString.substring(0, programString.indexOf('{')).trim();
		}
